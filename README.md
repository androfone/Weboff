...
class BrowserViewModel: ObservableObject {
        ...
        monitor.pathUpdateHandler = { path in
            if path.status == .satisfied {
                DispatchQueue.main.async {
                    self.handleNetworkChange(isOnline: true)
                }
            } else {
                DispatchQueue.main.async {
                    self.handleNetworkChange(isOnline: false)
                }
            }
        }
    ...
    func saveCurrentPage(html: String) {
        offlineHTML = html
        saveHTMLContent(url: currentURL, html: html)
    ...
    func loadOfflineContent(for url: String) {
        if let cachedHTML = loadCachedHTML(for: url) {
    ...
    func saveOfflinePage(for url: URL, html: String) {
        ...
        saveHTMLContent(url: url, html: localHTML)
    }
    ...
    func prefetchAndCacheContent(for url: URL) {
        if let html = loadCachedHTML(for: url.absoluteString) {
    ...
    func savePageMetadata(for url: URL) {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: metadata, options: []) {
            saveData(data, forKey: url.absoluteString)
        }
    }

    func loadPageMetadata(for url: URL) -> [String: Any]? {
        if let data = loadData(forKey: url.absoluteString) {
    ...
    func saveUserPreferences() {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: preferences, options: []) {
            saveData(data, forKey: "userPreferences")
        }
    }

    func loadUserPreferences() -> [String: Any]? {
        if let data = loadData(forKey: "userPreferences") {
            return (try? JSONSerialization.jsonObject(with: data, options: [])) as? [String: Any]
        }
        return nil
    }
    ...
    func saveCookies() {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: cookieData as Any, options: []) {
            saveData(data, forKey: "cookies")
        }
    }

    func loadCookies() {
        if let data = loadData(forKey: "cookies"),
    ...
    func saveUserData() {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: userData, options: []) {
            saveData(data, forKey: "userData")
        }
    }

    func loadUserData() -> [String: Any]? {
        if let data = loadData(forKey: "userData") {
            return (try? JSONSerialization.jsonObject(with: data, options: [])) as? [String: Any]
        }
        return nil
    }
    ...
    func saveLoginState() {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: loginState, options: []) {
            saveData(data, forKey: "loginState")
        }
    }

    func loadLoginState() -> [String: Any]? {
        if let data = loadData(forKey: "loginState") {
            return (try? JSONSerialization.jsonObject(with: data, options: [])) as? [String: Any]
        }
        return nil
    }
    ...
    func saveFormData() {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: formData, options: []) {
            saveData(data, forKey: "formData")
        }
    }

    func loadFormData() -> [String: Any]? {
        if let data = loadData(forKey: "formData") {
            return (try? JSONSerialization.jsonObject(with: data, options: [])) as? [String: Any]
        }
        return nil
    }
    ...
    func loadCachedHTML(for url: String) -> String? {
        let fileName = URL(string: url)?.lastPathComponent ?? "index.html"
    ...
    func downloadContent(from url: String) -> Data? {
    ...
    func saveCSS(_ html: String) {
        let cssPattern = "<link rel=[\"']stylesheet[\"'] href=[\"'](.*?)[\"'][^>]*>"
        if let regex = try? NSRegularExpression(pattern: cssPattern) {
            let matches = regex.matches(in: html, range: NSRange(location: 0, length: html.utf16.count))
            for match in matches {
                if let range = Range(match.range(at: 1), in: html) {
                    let url = String(html[range])
                    if let cssData = downloadContent(from: url) {
                        saveData(cssData, forKey: url)
                    }
                }
            }
        }
    ...
    func saveJavaScript(_ html: String) {
        let jsPattern = "<script src=[\"'](.*?)[\"'][^>]*></script>"
        if let regex = try? NSRegularExpression(pattern: jsPattern) {
            ...
            for match in matches {
                if let range = Range(match.range(at: 1), in: html) {
                    let url = String(html[range])
                    if let jsData = downloadContent(from: url) {
                        saveData(jsData, forKey: url)
                    }
                }
            }
        }
